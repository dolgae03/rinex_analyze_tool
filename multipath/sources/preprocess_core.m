function out_iphi = preprocess_core...
    (staid,in_t,in_iphi,in_irho,in_el,in_lli,con_arc,allow_arc_len,...
    allow_arc_pnt,slip_para,eff_slip_para,out_para,out_para_rho,smoothwin)
...
for arc=1:arc_num
    ...
    % smoothing of irho and leveling of iphi
    for n=1:size(arc2,1)
        valrows=find(t>=arc2(n,1) & t<=arc2(n,2));
%         iphi(valrows(1),3)=1;
        [Brs Brsig] = ...
            level_phi2rho(iphi(valrows,1),irho(valrows),el(valrows),smoothwin);
        iphi(valrows,1)=iphi(valrows,1)+Brs;
        iphi(valrows,2)=Brsig;
    end
end
